...
```bash
# Build Mac framework
xcodebuild -project Serve.xcodeproj -scheme "Serve (Mac)" -configuration Debug build

# Build iOS framework
xcodebuild -project Serve.xcodeproj -scheme "Serve (iOS)" -configuration Debug -destination 'generic/platform=iOS Simulator' build

# Build tvOS framework
xcodebuild -project Serve.xcodeproj -scheme "Serve (tvOS)" -configuration Debug -destination 'generic/platform=tvOS Simulator' build
```

## Project Structure

- `Sources/Serve/` - Core web server implementation
- `Sources/ServeUploader/` - File upload/download web interface
- `Sources/ServeWebDAV/` - WebDAV server implementation
...
- **Shape A lives or dies on not accumulating anything.** A descriptor or memory reservation
  ...
  in-memory endpoint until the process is relaunched. `+[SRVServer reservedInMemoryByteCount]`
...
- **Shape A also depends on large-file correctness.** Multi-hundred-megabyte builds pulled
  ...
  builds while they may be downloading — `SRVFileResponse` opening once and deriving
...
**Tailscale deployments must set `SRVOption_AllowedHostNames`** to the node's
...
`-[SRVGZipDecoder close:]` asserted `Z_STREAM_END` and then reported success
regardless; `SRV_DCHECK` is a no-op in Release. A `PUT` of a 20-byte gzip prefix over an
...
`-[SRVRequest ifRange]` and an initializer taking it). `filename*` was
...
**The recorded-trace corpus builds again.** The third pass's `__SERVE_ENABLE_TESTING__`
change also took the definition from the `SRVServer (Mac)` command-line target, which
...
**`addGETHandlerForBasePath:` had no containment check** (`SRVServer.m`) — the only
...
`SRVUploader` and this file recorded the bug class as fixed; WebDAV's nine sites still
...
**Digest nonce integrity tags were forgeable.** `SRVComputeMD5Digest` hashed via
...
**`SRVFileResponse` stat'd and opened as two separate path walks**, so a file
...
`kSRVMaxConnections` concurrent requests the real ceiling was their product — about
...
`kSRVMaxTotalInMemoryLength` (64 MB) bounds the sum across all live connections.
Every place that holds request data in memory takes a `SRVMemoryReservation` and
...
`SRVSetMemoryLimitsForTesting` shrinks the limits so a bound can be proven without
...
`SRVMaxInMemoryBodyLength()` / `SRVMaxDecompressedBodyLength()` rather than
the `kSRV...` constants, so the overrides are honoured.
...
`-[SRVConnection _rejectIfHostNotAllowed]`; anything else gets 421. Accepted with
no configuration: any IP address literal, `localhost`, this machine's own host name, and the
Bonjour name being advertised. `SRVOption_AllowedHostNames` adds more, and an entry
...
**Multipart argument accumulation (remote OOM).** `kSRVMaxInMemoryBodyLength`
...
before rejecting nothing). `SRVMIMEStreamParser` now carries a
`SRVMIMEStreamBudget` — shared with every sub-parser, since nested
`multipart/mixed` appends into the same arrays — capping total retained argument bytes at
`kSRVMaxInMemoryBodyLength` and total parts at `kMultiPartMaxParts` (1024, which
...
**`__SERVE_ENABLE_TESTING__` shipped in Release.**
...
(`X-SRVServer-CreationDate`/`-ModifiedDate` on PUT and MKCOL), a client-chosen
`X-SRVServer-LockToken`, and the request/response recording machinery. Now Debug-only.
...
**Malformed input aborted Debug builds.** `SRV_DNOT_REACHED()` is `abort()` under `#if
...
**Header parameters matched on substrings.** `SRVExtractHeaderValueParameter`
...
**gzip on an async-only response sent an empty body.** `SRVGZipEncoder` pulled its
source through the synchronous `readData:` only, so any `SRVStreamedResponse` with
`gzipContentEncodingEnabled` produced a valid gzip stream of zero bytes and never ran its
stream block. `SRVBodyEncoder` now implements `asyncReadDataWithCompletion:`.
...
and `SRVStreamedResponse` releases its block on `-close` to break handler retain
cycles.
...
`SRVErrorResponse`'s `_EscapeHTMLString` escaped only `"`, so
...
escaper in `SRVServer.m`. Covered by `testErrorResponseEscapesReflectedMarkup`.

### Per-connection config snapshot (auth/serverName race fix)

Each `SRVConnection` now **snapshots** the server's mutable configuration —
...
**In-memory body caps** (`kSRVMaxInMemoryBodyLength` = 16 MB,
`kSRVMaxDecompressedBodyLength` = 64 MB, in `SRVPrivate.h`).
...
- `SRVDataRequest` body (DAV PROPFIND/LOCK/MKCOL, forms, data requests)
- `SRVGZipDecoder` total inflated output — bails inside the inflate loop, so a zip bomb can't balloon the buffer first
...
### Server-Sent Events (SSE) for SRVUploader

Added live browser updates when files change on the device.

**Files modified:**
- `Sources/ServeUploader/SRVUploader.h` - Added `serverSentEventsEnabled` property
- `Sources/ServeUploader/SRVUploader.m` - SSE infrastructure implementation
- `Sources/ServeUploader/SRVUploaderSSEChannel.h` - Per-connection SSE buffer (state machine)
- `Sources/ServeUploader/SRVUploader.bundle/Contents/Resources/js/index.js` - EventSource client

**Reliability model (per-connection buffering):**
SRVServer's async streaming API is a strict ping-pong — it hands the response
...
connection now owns a `SRVUploaderSSEChannel` that buffers events in FIFO
...
must call `-[SRVUploaderSSEChannel close]`, which completes any parked
reader with empty data — SRVServer's end-of-stream sentinel — and makes
...
`SRVOption_ConnectionIdleTimeout` (NSNumber / double, default 30.0
...
- `SRVOption_AutomaticallySuspendInBackground: false`
