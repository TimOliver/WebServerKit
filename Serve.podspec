Pod::Spec.new do |s|
  ...
  s.homepage = 'https://github.com/TimOliver/Serve'
  s.summary  = 'Embedded HTTP, WebDAV and file-upload server for macOS, iOS and tvOS. A hardened fork of GCDWebServer.'

  s.source   = { :git => 'https://github.com/TimOliver/Serve.git', :tag => s.version.to_s }
  ...
  # OTHER_LDFLAGS in Serve.xcodeproj), so the declarations below are not
  # per-platform either.
  s.subspec 'Core' do |cs|
    cs.source_files = 'Sources/Serve/**/*.{h,m}'
    # include/ holds symlinks to the public headers so SwiftPM can expose them as the
    # Serve module (see Package.swift). CocoaPods reads the real files directly and
    # would otherwise see each header twice.
    cs.exclude_files = 'Sources/Serve/include/**/*'
    cs.private_header_files = 'Sources/Serve/Core/SRVPrivate.h'
  ...
  s.subspec 'WebDAV' do |cs|
    cs.dependency 'Serve/Core'
    cs.source_files = 'Sources/ServeWebDAV/*.{h,m}'
    cs.requires_arc = true
    cs.library = 'xml2'
  ...
  s.subspec 'WebUploader' do |cs|
    cs.dependency 'Serve/Core'
    cs.source_files = 'Sources/ServeUploader/*.{h,m}'
    # Implementation detail of the SSE endpoint, not part of the public API.
    cs.private_header_files = 'Sources/ServeUploader/SRVUploaderSSEChannel.h'
    cs.requires_arc = true
    cs.resources = 'Sources/ServeUploader/SRVUploader.bundle'
  end
end
