...
let server = WSKWebServer()
let uploader = WSKWebUploader(uploadDirectory: directory)   // reaches the resource accessor
let dav = WSKWebDAVServer(uploadDirectory: directory)
print("linked: \(type(of: server)), \(type(of: uploader)), \(type(of: dav)); reserved=\(WSKWebServer.reservedInMemoryByteCount)")
